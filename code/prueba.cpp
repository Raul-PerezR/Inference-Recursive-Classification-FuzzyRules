...
vector<vector<double> > CalculoEstimadoSobrePatronesConNEtiquetas(int nlabels, example_set & E, const VectorVar &V, int num_par,
  ...
  double media_patrones = 0;
	...
	for (int par=0; par<num_par; par++){
		...
		example_set E_Par_Completo=E.Extract_Training_Set(par, V.Consecuente(), 0);
		example_set E_Par_Test = E.Extract_Test_Set(par, V.Consecuente());
    cout << "number examples in test set: " << E_Par_Test.N_Examples() << endl;
    ...
    double t_uno = 0;
		...
		for (int t=0; t<partes; t++){ // Numero de trozos
		    ...
		    int clase_predicha = -1;
				...
				step_begin = clock();
        int Tries = NumMaxEvaluatedRules;
				...
				switch (model) {
					...
					case 2: clase_predicha = Patrones2.TestearPatronesBasicosClassic_UnEjemplo(E_Par_Test,V2,i,pesoR,claseR, grado,antecedenteSeleccionado);
						      break;
				  case 3: Patrones2.TestearRecursivoUnEjemplo(E_Par_Test, V2, i, regla, 0, 1, umbral, clase_predicha, PCF, antecedenteSeleccionado, Tries);
								  grado = umbral;
                  //cout << i << "\t" << antecedenteSeleccionado << "|" << clase_predicha << endl << endl;
									break;
					case 4: clase_predicha = Patrones2.InferenciaRecursivaOptimalizada(E_Par_Test,V2,i,PCF,grado,antecedenteSeleccionado, Tries);
                  //cout << i << "\t" << antecedenteSeleccionado << "|" << clase_predicha << endl << endl;
					        break;
					...
					case 6: if (clase_predicha == -1)
										...
										if (clase_predicha == -1)
											clase_predicha = Patrones2.InferenciaRecursivaOptimalizada(E_Par_Test,V2,i,PCF,grado,antecedenteSeleccionado, Tries);
										if (clase_predicha == -1)
    ...
    cout << "\tEx " << 100
         << "\%  ETA: " << PonerTiempo(0)
         << "   One: " << t_uno / CLOCKS_PER_SEC
         << "   TotalTime: " <<  PonerTiempo(t_uno *nex/CLOCKS_PER_SEC) << endl;
